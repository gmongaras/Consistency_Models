...
def init_fn(weight):
  nn.init.normal_(weight, 0, 2.0)
conv_kernel_init_fn = init_fn
...
def dense_layer_init_fn(key,
  ...
  return torch.distributions.uniform.Uniform(0, 1).sample(shape).to(dtype) * unif_init_range
...
#   gate_forward_key, gate_backward_key = jax.random.split(rng, num=2)
  ...
  gate_forward = torch.distributions.uniform.Uniform(0, 1).sample(gate_shape).to(torch.float32)
  gate_backward = torch.distributions.uniform.Uniform(0, 1).sample(gate_shape).to(torch.float32)
...
def shake_drop_train(x,
  ...
  mask = torch.distributions.bernoulli.Bernoulli(mask_prob).sample(rnd_shape).to(torch.float32)

  alpha_values = torch.distributions.uniform.Uniform(alpha_min, alpha_max).sample(rnd_shape).to(torch.float32)
  beta_values = torch.distributions.uniform.Uniform(alpha_min, alpha_max).sample(rnd_shape).to(torch.float32)
  ...
  return x * rand_backward + (
    x * rand_forward - x * rand_backward).detach()
...
def _output_add(block_x, orig_x):
  ...
  if block_x.shape[-1] != orig_x.shape[-1]:
    ...
    orig_x = F.pad(orig_x, [(0, 0), (0, 0), (0, 0), (0, channels_to_add)])
...
class GaussianFourierProjection(nn.Module):
  ...
  @nn.compact
  def __call__(self, x):
    W = torch.ParameterDict({"W": torch.empty(self.embedding_size).normal_(mean=0,std=self.scale)}).detach()
    x_proj = x[:, None] * W[None, :] * 2 * torch.pi
...
class WideResnet(nn.Module):
  ...
  def __call__(self, x, sigmas, train=True):
    ...
    N = np.prod(x.shape[1:])
    x = (x - torch.mean(x, axis=(1, 2, 3), keepdims=True)) / torch.maximum(torch.std(x, axis=(1, 2, 3), keepdims=True),
                                                                       1. / np.sqrt(N))
    temb = GaussianFourierProjection(embedding_size=128, scale=16)(torch.log(sigmas))
